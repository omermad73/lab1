...
class Switch(GNO):
    def __init__(self, num_ports, mac_table_size, log_file=None, ttl=10):
        super().__init__("Switch")
        if not (num_ports & (num_ports - 1) == 0) and num_ports != 0:
            raise ValueError("Number of ports must be a power of 2.")
        if mac_table_size > 10:
            raise ValueError("MAC table too long, the limit size is 10")
        self.num_ports = num_ports
        self.ports = [None] * num_ports
        self.ttl = ttl  # Time-to-live for MAC table entries
        self.mac_table = [{'used': False, 'mac': None, 'port': None, 'time': None} for _ in range(mac_table_size)]
